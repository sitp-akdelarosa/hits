...
<%
	...
	Select Case  str_show_column
		...
		Case "2"
			...
			ReDim FieldKey(17)
			FieldKey=Array("搬入票出力","搬出入予定日","指示元","指示元へ回答","回答","作業番号","コンテナ番号/BL番号","船社","船名","SZ","搬入元","CY","搬出許可日","フリータイム","CYカット日","指示先","指示先回答")		
...
function fInit(){
	...
	if("<%=str_show_column%>" == "1"){ 
		...
		for(i=0;i<17;i++){                            //Y.TAKAKUW Upd from 17
			fCheck(str,i,"Chk_Sel"+i,"Chk_SelDis"+i);
		}
	}
}
...
function fFormatPage(){
	...
	if("<%=str_show_column%>" == "1"){  
		...
		createCookie('HitsTbl2', str, 17) //Y.TAKAKUWA Upd from 17
	}
...
function fClear(){
	...
	if("<%=str_show_column%>" == "1"){  
		...
		createCookie('HitsTbl2', "", 17) //Y.TAKAKUW Upd from 17
	}
	opener.document.frm.submit();
	window.close();
}
