...
Function getSort(Key,SortKey,str)
		...
		elseif str <> "" AND Key<>"" Then 
		    str = str & ","
		elseif str = "" AND Key = "" then
		    str =" ORDER BY MAXDATE DESC,InputDate ASC, BookNo ASC "		
		end if
		if Key <> "" then 
...
'  DtTbl(0)=Array("入力日","指示元","ブッキング番号","ピック済本数","指示先","指示先回答","照会フラグ","作業完了F","船社","船名","指示元表示用","備考１","指示先データ","備考２","サイズ","タイプ","高さ","材質")
  ...
  DtTbl(0)=Array("入力日","指示元","ブッキング番号","ピック<BR>済","指示先","指示先<BR>回答","照会フラグ","作業完了F","船社","船名","指示元表示用","備考１","指示先データ","備考２","SZ","タイプ","H","材質","CYカット日","空コン搬出先","コード","担当","コード","担当")
'Chang 20050303 END
