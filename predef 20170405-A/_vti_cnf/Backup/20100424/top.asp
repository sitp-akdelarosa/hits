...
<%
	...
	if strSort <> "" then			
		...
		if Mid(strSort,4,2) = "XX" then
			...
			Session("TB2Key2") = Mid(strSort,3,2)
		end if
		...
		if Mid(strSort,7,2) = "XX" then
			...
			Session("TB2Key3") = Mid(strSort,5,2)
		end if
...
'			 strWhere & strOrder1		
	...
	StrSQL = "SELECT ISNULL(CONVERT(varchar(10),SPB.InputDate,111),'') as InputDate,"&_
			 ...
			 " LEFT JOIN mVessel AS mV ON BOK.VslCode = mV.VslCode"&_
			 " WHERE (SPB.SenderCode='"& USER &"' OR SPB.TruckerCode='"& COMPcd &"') AND SPB.Process='R'"&_
			 "  AND '"&calcDate&"' <= SPB.InputDate AND (SPB.TruckerFlag='0' OR SPB.TruckerFlag=' ') " & strWhere			 
...
Function getDataTbl2()
	...
	StrSQL ="SELECT  * FROM (SELECT T.* FROM (SELECT 'FULLOUT' As Type,ITC.DeliverTo,ITC.BLNo, " & _
		...
		"OR ITC.TruckerSubCode1='"& COMPcd &"' "&_
			...
			"AND ('"&calcDate1&"' <= ITC.WorkCompleteDate Or ITC.WorkCompleteDate IS Null) "&_
			"AND (DateDiff(day,ITC.WorkDate,'"&Date&"')<=0 Or ITC.WorkDate IS Null) "&_
		"UNION ALL "&_
					...
					<table border="1" cellpadding="0" cellspacing="0" width="100%">										
						...
						<tbody>							
							...
							<tr class=bgw>													
								...
								<td id="D1Col05" nowrap><A HREF="JavaScript:GoRenewEmpty('<%=Trim(ObjRS("BookNo"))%>','<%=Trim(ObjRS("NumCount"))%>','<%=Trim(ObjRS("SenderCode"))%>','<%=Trim(ObjRS("TruckerCode"))%>','<%=Trim(ObjRS("ShipLine"))%>');"><%=Trim(ObjRS("BookNo"))%></A><BR></td>
