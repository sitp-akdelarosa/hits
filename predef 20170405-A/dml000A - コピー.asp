...
'
'  （変更履歴）
'   2013-02-14   Y.TAKAKUWA   作成
'   2013-07-24   Y.TAKAKUWA   Filter SQL by IC and KA.
'   2013-09-25   Y.TAKAKUWA   ロギングの機能を追加
'**********************************************
Option Explicit
Response.Expires = 0
'HTTPコンテンツタイプ設定
...
Function getDataTbl2()
	...
	'WriteLogH "", "ロックオン事前情報一覧", "", ""
	'2013-09-25 Y.TAKAKUWA Add-S 
	WriteLogH "b501", "コンテナロック対象一覧表示", "01",""
	'2013-09-25 Y.TAKAKUWA Add-S
	...
	HeaderTbl2(0) = "選択"	
	HeaderTbl2(1) = "ID入力"	
	HeaderTbl2(2) = "ドライバ名称<BR/>/グループID"
	HeaderTbl2(3) = "ロック中<BR/>ヘッドID"
	HeaderTbl2(4) = "搬出入<BR/>予定日"		
...
function fRelease(){
	...
	if(updReleaseFlg ==1){
	   var msg = confirm("コンテナロック指示を解除します。よろしいですか？");
       if(msg == true){
	   ...
	   if(updReleaseFlg ==2){
	     var msg = confirm("コンテナロック中の作業がありますがよろしいですか？");
         if(msg == true){
           document.frm.Gamen_Mode.value = "R";
           document.frm.submit();
         }
...
<table border="0" cellpadding="0" cellspacing="0">
      ...
      <td width="100%" height="30">
        <div style="margin-left:30px;">
          <table border="0" cellpadding="0" cellspacing="0">
		      ...
		      <td width="115" align="center" nowrap><a HREF="JavaScript:OpenWin(1)">ドライバ承認</a></td>
		      <td width="115" align="center" nowrap>
		        <a HREF="JavaScript:OpenWin(2)">ドライバ一覧と削除</a>
              </td>
		      <td align="center" nowrap style="width: 115px"><a href="JavaScript:OpenWin(3)">グループ登録</a></td>
		         ...
		         <tr>
		           <td>
					  ...
					  <% If v_InOutFlag="1" Then %>
						<OPTION VALUE = ''>搬入＆搬出</OPTION>
						<OPTION VALUE = '1' SELECTED>搬入のみ</OPTION>
						<OPTION VALUE = '2'>搬出のみ</OPTION>
					  <% ELSEIf v_InOutFlag="2" Then %>
						<OPTION VALUE = ''>搬入＆搬出</OPTION>
						<OPTION VALUE = '1'>搬入のみ</OPTION>
						<OPTION VALUE = '2' SELECTED>搬出のみ</OPTION>
					  <% ELSE  %>
						<OPTION VALUE = '' SELECTED>搬入＆搬出</OPTION>
						<OPTION VALUE = '1'>搬入のみ</OPTION>
						<OPTION VALUE = '2'>搬出のみ</OPTION>
					  <% END IF  %>
				     </Select>
		           ...
		           <td>
		              <input type="button" name="Button"  value="表示更新" onClick="fRSearch()">
		           </td>
		         </tr>
		         </table>
		      </td>
                    ...
                    <table border="1" cellpadding="0" cellspacing="0" width="100%" id="TBInOut">						
						<tbody>
						     <!--DETAIL INFORMATION START-->
							  ...
							  <%If Trim(ObjRS2("WorkDate")) = "" Then  %>
							    <td id="D2Col07" nowrap style="color:Red;width:80px;">未入力<BR /></td>
							  <%else%>
	...
	<tr>		
		<td>
		    <div style="margin-left:50px;">
			  ...
			  <tr>
			    <td><input type="button" name="Button"  value="入力したIDで登録" onClick="LockOnReg()"></td>
			  </tr>
			  </table>	
			</div>		
		</td>
	...
	<tr>		
		<td>
		  <div id="BDIV2">
		    <DIV style="width:100px; padding:10px;background-color:#FFCCFF; text-align:center;">選択して登録</DIV>
		      <Br />
		      <div style="margin-left:30px;">
		        <table border="0" cellpadding="2" cellspacing="0">
			    <tr>
			      <td> 表で選択した作業に対し、ドライバ、または、グループで指示します。</td>
			    </tr>
			    <tr>
			      <td>
	...
	<tr>
		<td>
		  <div style="margin-left:50px;">
			<table border="0" cellpadding="2" cellspacing="0">
			<tr>
			<td><input type="button" name="Button"  value="登録" onClick="fGrpUpd()"></td>
			</tr>
			</table>
		  </div>
		</td>	
	...
	<tr>
		<td>
		  <DIV style="width:100px; padding:10px;background-color:#FFCCFF; text-align:center;">選択して解除</DIV>
		  <Br />
		  <div style="margin-left:30px;">
		    <table border="0" cellpadding="2" cellspacing="0">
			<tr>
			<td> 表で選択した作業に対し、コンテナロック指示やドライバのコンテナロックを解除します。</td>
			</tr>
			</table>
		  </div>
		</td>
	...
	<tr>
		<td>
		  <div style="margin-left:50px;">
			<table border="0" cellpadding="2" cellspacing="0">
			<tr>
			<td><input type="button" name="Button"  value="解除" onClick="fRelease()"></td>
			</tr>
			</table>
		  </div>
		</td>
