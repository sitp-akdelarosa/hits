...
''    CheckLogin "expentry.asp"
    ...
    WriteLog fs, "9999","輸出コンテナ照会(外部)","Start", ""

    ' Tempファイル属性のチェック
    CheckTempFile "IMPORT", "impentry.asp"
    WriteLog fs, "9999","輸出コンテナ照会(外部)","1", ""
    ...
    WriteLog fs, "9999","輸出コンテナ照会(外部)","2", ""

    ' 指定引数の取得
    Dim strRequest
    strRequest = Request.QueryString("request")  ' 更新リクエスト画面ID
    WriteLog fs, "9999","輸出コンテナ照会(外部)","3", ""
...
'    Set fs=Server.CreateObject("Scripting.FileSystemObject")
    ...
    WriteLog fs, "9999","輸出コンテナ照会(外部)",strFileName, ""
    If strFileName="" Then
    WriteLog fs, "9999","輸出コンテナ照会(外部)","セッション切れた", ""
        ' セッションが切れているとき
    ...
    ' データベースの接続
    ConnectSvr conn, rsd
    WriteLog fs, "9999","輸出コンテナ照会(外部)","4", ""

    ' 転送ファイルのOpen
    Set ti=fs.OpenTextFile(Server.MapPath(strFileName),2,True)
    WriteLog fs, "9999","輸出コンテナ照会(外部)","5", ""
...
'☆☆☆ Del_E  by nics 200902改造
    ...
    ti.Close
    conn.Close
    WriteLog fs, "9999","輸出コンテナ照会(外部)","6", ""

    ' 詳細画面からのとき、該当コンテナデータの行を検索する
    If strRequest="impdetail.asp" Then
    WriteLog fs, "9999","輸出コンテナ照会(外部)","7", ""
        ' 記憶している検索条件をロード
    ...
    WriteLog fs, "9999","輸出コンテナ照会(外部)","8", ""
        ' 該当レコードないとき
        bError = true
...
'    WriteLog fs, "輸入コンテナ照会", "画面更新"
    WriteLog fs, "9999","輸出コンテナ照会(外部)","9", ""
...
End of comment by seiko-denki 2003.07.18 -->
      ...
      <table>
        <tr> 
          <td nowrap>
            <dl> 
            <dt><font color="#000066" size="+1">【コンテナ情報照会用ファイル転送画面】</font><br>
            <dd>
<%
    ' エラーメッセージの表示
