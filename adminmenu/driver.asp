...
'
'   2017/01/17    T.Okui         選択したドライバを削除する機能を修正
'********************************************************************
...
Function LfGetRequestItem()
    ...
    '20170118 T.Okui Start
	For i = 1 to (v_DataCnt2) - 1 
	    Arr_Check(i) = Trim(Request.form("chkInOut" & i))
        Arr_DriverID(i) = TRIM(Split(Request.form("LODriverID" & i),",")(0))
	Next
...
function view(){
    ...
    obj3header.style.width=obj2.clientWidth;
	obj3header.style.height = rowHeightThead;
...
function fDelete(){
    ...
    for(i=1;i<=parseInt(document.frm.DataCnt2.value)-1; i++){
      if(document.getElementsByName("chkInOut" + i)[0].checked == true){
         chkFlag = 1;
      }
    }
