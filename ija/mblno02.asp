...
If sErrMsg<>"" Then
	WriteLogM oFS, "Unknown", "2203", "携帯-BL番号照会", "10",sPhoneType, sBLNo & "," & "入力内容の正誤:1(誤り)" & sErrMsg
Else
	WriteLogM oFS, "Unknown", "2203", "携帯-BL番号照会", "10",sPhoneType, sBLNo & "," & "入力内容の正誤:0(正しい)"
	WriteLogM oFS, "Unknown", "2204", "携帯-コンテナ番号一覧", "00",sPhoneType, nPageNo & "/" & nPageMax & ","
End If
Set oFS = Nothing
...
%>
			...
			<a task="gosub" accesskey="<%=CStr(nContPtr Mod 10)%>"
						dest="mcont02.asp?Ctno=<%=sContNo(nContPtr)%>">
				<%=sContNo(nContPtr)%>
			</a><br>
<%
		Next
...
<%
		If 1 < nPageNo Then
%>
			<a task="gosub" dest="mblno02.asp?PAGENO=<%=nPageNo - 1%>&BLno=<%=sBlno%>">前頁</a>
<%
		End If
		If nPageNo < nPageMax Then
%>
			<a task="gosub" dest="mblno02.asp?PAGENO=<%=nPageNo + 1%>&BLno=<%=sBlno%>">次頁</a>
<%
		End If
%>
		<br>
...
<%
	Else
		For nContPtr = 1 To nContCount
%>
			<a href="mcont02.asp?Ctno=<%=sContNo(nContPtr)%>" <%=GetKeyTag(CStr(nContPtr))%>>
...
<%
		If 1 < nPageNo Then
%>
			<a href="mblno02.asp?PAGENO=<%=nPageNo - 1%>&BLno=<%=sBlno%>">前頁</a>
<%
		End If
		If nPageNo < nPageMax Then
%>
			<a href="mblno02.asp?PAGENO=<%=nPageNo + 1%>&BLno=<%=sBlno%>">次頁</a>
<%
		End If
%>
		<br>
