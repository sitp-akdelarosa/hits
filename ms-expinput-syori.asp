...
End Function
    ...
    iLine=Session.Contents("editline")
    ...
    If Not bError Then
        ...
        If strKind="1" Then
            sEXxx = iSeqNo & "," & sTranID & "," & sSyori & ","  & sTusin & ",Web - " & _
                    sSosin & "," & sPlace & "," & anyTmp(40) & "," &  anyTmp(21) & "," & _
                    anyTmp(1) & "," & anyTmp(0) & "," & anyTmp(41) & "," & sDate & "," & sSosin
        Else
            sEXxx = iSeqNo & "," & sTranID & "," & sSyori & ","  & sTusin & ",Web - " & _
                    sSosin & "," & sPlace & "," & anyTmp(40) & "," &  anyTmp(21) & "," & _
                    anyTmp(1) & "," & anyTmp(0) & "," & anyTmp(41) & "," & sDate & ",,,,," & sSosin
        ...
        ' テンポラリファイル更新
        strTemp=Left(sDate,4) & "/" & Mid(sDate,5,2) & "/" & Mid(sDate,7,2) & " " & Mid(sDate,9,2) & ":" & Mid(sLogDate,11,2)
        If strKind="1" Then
            anyTmp(47)=strTemp
        Else
            anyTmp(48)=strTemp
        ...
        ' 海貨入力項目選択
        If strKind="1" Then
            WriteLog fs, "","輸出入業務支援-輸出空コンテナ倉庫到着時刻入力","10", strOption
        Else
            WriteLog fs, "1107","輸出コンテナ照会-バンニング完了時刻入力","10", strOption
        End If
...
<%
    ...
    strOption = anyTmp(0) & "/" & sLogDate & "," & "入力内容の正誤:1(誤り)"
    ' 海貨入力項目選択
    If strKind="1" Then
        WriteLog fs, "","輸出入業務支援-輸出空コンテナ倉庫到着時刻入力","10", strOption
    Else
