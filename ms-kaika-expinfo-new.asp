...
<%
	...
	Dim sUser,sUserNo,sVslCode,sVoyCtrl,sBooking,sTraderCode,sArvTime,sCyDate,sSize,sType,sHeight,sPick,sRemark
	If Not bKind=1 Then
...
<SCRIPT Language="JavaScript">
	...
	function checkFormValue(){
		...
		if(!checkBlank(getFormValue(3))){ return showAlert("荷主管理番号",true); }
		if(!checkBlank(getFormValue(4))){ return showAlert("Booking No.",true); }
		...
		if(!checkDate(new getDateValue(getFormValue(11),getFormValue(12),getFormValue(13)))){
			return showAlert("ＣＹ搬入指定日",false);
		}

		if(!checkNumberFormat(getFormValue(14))){ return showAlert("サイズ",false); }
		if(!checkNumberFormat(getFormValue(16))){ return showAlert("高さ",false); }
      ...
      <table>
        <tr>
          <td nowrap align=center>
            ...
            <form method=post name="input" action="ms-kaika-expinfo-exec.asp">
              <center>
...
<%  End If %>
                ...
                <tr> 
                  ...
                  <td nowrap>
<%
	Dim eyear,emon,eday,ehour,emin,cyyear,cymon,cyday
	If bKind=0 Then
		If sArvTime<>"" Then
		...
		End If
		If sCyDate<>"" Then
			cyyear= Left(sCyDate,4)
			cymon = Mid(sCyDate,6,2) 
			cyday = Mid(sCyDate,9,2) 
		End If
	End If
%>
                ...
                <tr> 
                  ...
                  <td nowrap>
                    <input type=text name=cydate_year value="<%=cyyear%>" size=5 maxlength=4>年
                    <input type=text name=cydate_mon value="<%=cymon%>" size=3 maxlength=2>月
                    <input type=text name=cydate_day value="<%=cyday%>" size=3 maxlength=2>日<BR>
					<table border=0 cellpadding=0 cellspacing=0>
					  <tr>
						<td width=200>
...
<% ElseIf bKind=2 Then %>
                <input type=button value=" 中  止 " onClick="JavaScript:window.location.href='ms-kaika-expinfo-list.asp'">
<% Else %>
...
<%
	...
	If bKind=0 Then
		'更新
	    WriteLog fs, "4102","海貨入力輸出貨物情報-情報入力", "02", sBooking & ","
	Else
		'新規
	    WriteLog fs, "4102","海貨入力輸出貨物情報-情報入力", "01", ","
	End If
	' Log作成
%>
