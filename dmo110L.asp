...
'2009/02/25 Add-S G.Ariola   
  ...
  FieldName=Array("WorkDate","Code1","Name1","ContNo","ShipLine"," FullName"," ContSize","ReturnPlace","DetentionFreeTime","Code2","Flag1","Comment1")
...
'20030911  DtTbl(0)=Array("入力日","搬入予定日","指示元","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元への回答","船社","船名","サイズ","返却先","ディテンション<BR>フリータイム","備考")
  DtTbl(0)=Array("入力日","搬入<BR>予定日","指示元","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元への回答","船社","船名","SZ","返却先","ディテンション<BR>フリータイム","備考","作業管理番号","コード","担当")
...
'3th Add End
  ...
  'データ取得
    StrSQL = "SELECT T.* FROM (SELECT ITC.InputDate, ITC.WorkDate, ITC.RegisterCode, ITC.TruckerSubCode1, ITC.TruckerSubCode2, "&_
			 ...
			 "(CASE "&_
			"WHEN ITC.TruckerSubCode1 = '"& COMPcd &"' THEN ITC.RegisterCode "&_
			...
			"WHEN ITC.TruckerSubCode4 = '"& COMPcd &"' THEN '' "&_
			...
			"ELSE ITC.TruckerSubCode1 END) as Code2, "&_
			"(CASE "&_
			"WHEN ITC.TruckerSubCode1 = '"& COMPcd &"' THEN ITC.TruckerSubName2 "&_
			"WHEN ITC.TruckerSubCode2 = '"& COMPcd &"' THEN ITC.TruckerSubName3 "&_
			"WHEN ITC.TruckerSubCode3 = '"& COMPcd &"' THEN ITC.TruckerSubName4 "&_
...
<%If Num<>0 Then%> 
  <% If DtTbl(0)(8)=0 Then %>
  <TR class=bga>
    <TH width="60" rowspan="2"><%=DtTbl(0)(1)%></TH><TH width="120" colspan="2"><%=DtTbl(0)(2)%></TH>
    <TH width="100" rowspan="2"><%=DtTbl(0)(3)%></TH><TH width="40" rowspan="2"><%=DtTbl(0)(9)%></TH><TH width="105" rowspan="2"><%=DtTbl(0)(10)%></TH>
    <TH width="20" rowspan="2"><%=DtTbl(0)(11)%></TH><TH width="120" rowspan="2"><%=DtTbl(0)(12)%></TH><TH width="110" rowspan="2"><%=DtTbl(0)(13)%></TH>
    <TH width="60" rowspan="2"><%=DtTbl(0)(5)%></TH><TH width="60" rowspan="2"><%=DtTbl(0)(6)%></TH><TH width="80" rowspan="2"><%=DtTbl(0)(14)%>
  ...
  <!--2009/02/25 Add-S G.Ariola test1 -->
  	<TR class=bga>
	<TH width="60"><%=DtTbl(0)(16)%></TH>
	<TH width="60"><%=DtTbl(0)(17)%></TH>
...
<TABLE border="1" cellPadding="2" cellSpacing="0" cols="<%=Num+20%>"><!--2009/02/25 G.Ariola -->    
    <% For j=1 to Num %>
  <TR class=bgw>
    <TD width="60"><%=DtTbl(j)(1)%><BR></TD><TD width="57"><%=DtTbl(j)(2)%><BR></TD><TD width="57"><%=Left(DtTbl(j)(16),4)%><!--<INPUT value="" type=text class=chrReadOnly size="9" readonly=TRUE tabindex = -1 > --><BR></TD>
    <TD width="100"><A HREF="JavaScript:GoRenew('<%=DtTbl(j)(3)%>','<%=DtTbl(j)(15)%>');"><%=DtTbl(j)(3)%></A></TD>
    <TD width="40"><%=DtTbl(j)(9)%><BR></TD><TD width="105"><%=Left(DtTbl(j)(10),12)%><BR></TD><TD width="20"><%=DtTbl(j)(11)%><BR></TD>
    <TD width="120"><%=DtTbl(j)(12)%><BR></TD><TD width="110"><%=DtTbl(j)(13)%><BR></TD>
    <TD width="60"><%=DtTbl(j)(5)%><BR></TD><!--<TD width="57"><INPUT value="<%=DtTbl(j)(17)%>" type=text class=chrReadOnly size="9" readonly=TRUE tabindex = -1 ><BR></TD> --><TD width="60"><%=DtTbl(j)(6)%></TD><TD width="80"><%=Left(DtTbl(j)(14),10)%><BR>
...
<%If Num>10 Then%></DIV><%end if%>	<!--2009/02/25 G.Ariola -->		
  ...
  <TR class=bga>
    <TH width="60" rowspan="2"><%=DtTbl(0)(1)%></TH><TH width="120" colspan="2"><%=DtTbl(0)(2)%></TH>
    <TH width="56" rowspan="2">指示元<BR>への回答</TH>
    <TH width="100" rowspan="2"><%=DtTbl(0)(3)%></TH><TH width="40" rowspan="2"><%=DtTbl(0)(9)%></TH><TH width="105" rowspan="2"><%=DtTbl(0)(10)%></TH>
    <TH width="20" rowspan="2"><%=DtTbl(0)(11)%></TH><TH width="120" rowspan="2"><%=DtTbl(0)(12)%></TH><TH width="110" rowspan="2"><%=DtTbl(0)(13)%></TH>
    <TH width="60" rowspan="2"><%=DtTbl(0)(5)%></TH><TH width="60" rowspan="2"><%=DtTbl(0)(6)%></TH><TH width="80" rowspan="2"><%=DtTbl(0)(14)%>
    <INPUT type=hidden name='Datatbl0' disabled value='<%=DtTbl(0)(0)%>,<%=DtTbl(0)(1)%>,<%=DtTbl(0)(2)%>,<%=DtTbl(0)(3)%>,<%=DtTbl(0)(4)%>,<%=DtTbl(0)(5)%>,<%=DtTbl(0)(6)%>,<%=DtTbl(0)(7)%>,<%=DtTbl(0)(8)%>,<%=DtTbl(0)(9)%>,<%=DtTbl(0)(10)%>,<%=DtTbl(0)(11)%>,<%=DtTbl(0)(12)%>,<%=DtTbl(0)(13)%>,<%=DtTbl(0)(14)%>'>
...
<%If Num>10 Then%>  <DIV id="VDIV" style=" width:848; height:242; overflow-y:auto;"><!--2009/02/25 G.Ariola -->
 ...
 <%end if%> <!--2009/02/25 G.Ariola -->
  ...
  <TR class=bgw>
		<TD width="60"><%=DtTbl(j)(1)%><BR></TD><TD width="57"><%=DtTbl(j)(2)%><BR></TD><TD width="57"><%=Left(DtTbl(j)(16),4)%><!--<INPUT value="" type=text class=chrReadOnly size="9" readonly=TRUE tabindex = -1 > --><BR></TD>
    <TD width="56"><%=DtTbl(j)(8)%></TD> 
    <TD width="100"><A HREF="JavaScript:GoRenew('<%=DtTbl(j)(3)%>','<%=DtTbl(j)(15)%>');"><%=DtTbl(j)(3)%></A></TD>
    <TD width="40"><%=DtTbl(j)(9)%><BR></TD><TD width="105"><%=Left(DtTbl(j)(10),12)%><BR></TD>
    <TD width="20"><%=DtTbl(j)(11)%><BR></TD><TD width="120"><%=DtTbl(j)(12)%><BR></TD><TD width="110"><%=DtTbl(j)(13)%><BR></TD>
    <TD width="57"><%=DtTbl(j)(5)%><BR></TD><!--<TD width="57"><INPUT value="<%=DtTbl(j)(17)%>" type=text class=chrReadOnly size="9" readonly=TRUE tabindex = -1 ><BR></TD> --><TD width="60"><%=DtTbl(j)(6)%></TD><TD width="80"><%=Left(DtTbl(j)(14),10)%><BR>
    <INPUT type=hidden name='Datatbl<%=j%>' disabled value='<%=DtTbl(j)(0)%>,<%=DtTbl(j)(1)%>,<%=DtTbl(j)(2)%>,<%=DtTbl(j)(3)%>,<%=DtTbl(j)(4)%>,<%=DtTbl(j)(5)%>,<%=DtTbl(j)(6)%>,<%=DtTbl(j)(7)%>,<%=DtTbl(j)(8)%>,<%=DtTbl(j)(9)%>,<%=DtTbl(j)(10)%>,<%=DtTbl(j)(11)%>,<%=DtTbl(j)(12)%>,<%=DtTbl(j)(13)%>,<%=DtTbl(j)(14)%>'>
    </TD>
  </TR>
    <% Next %>
