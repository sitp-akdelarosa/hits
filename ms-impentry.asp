...
<%
	...
	End If
    ' 輸入入業務支援-輸入コンテナ照会
    WriteLog fs, iNum,"輸入コンテナ照会-" & strUserKind & "用照会" ,"00", ","
%>
...
%>
      ...
      <table width="480">
        <tr>
          <td colspan="4">
			条件を入力しないで輸入照会ボタンを押すと、全てのデータが表示されます。<BR><BR>
...
%>
      <form name="con" method="get" action="ms-impcntnr.asp" onSubmit="return ClickSend()">
<%
    Else
%>
      <form name="con" method="get" action="ms-impcntnr.asp">
<%
    End If
%>
                    ...
                    <tr>
						<td>
							<table border=0 cellpadding=0 cellspacing=0>
							  <tr>
								<td width=150>
									<input type=text name=vessel size=10 maxlength="7">
								</td>
                    ...
                    <tr>
						<td>
							<table border=0 cellpadding=0 cellspacing=0>
							  <tr>
								<td width=150>
									<input type=text name=voyage size=18 maxlength="12">
								</td>
								<td align=right valign=middle nowrap>
									<font size=1 color="#2288ff">[半角英数]</font>
								</td>
