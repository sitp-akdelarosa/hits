...
function ClickDel() {
		...
		if (!ChkChara(document.UPD.DeliverTo.value)) {
			window.alert("コンテナ搬出先は英字で入力して下さい。");
			return false;
		}
}
function ClickRec() {
		...
		if (!ChkChara(document.UPD.ReceiveFrom.value)) {
			window.alert("コンテナ搬入元は英字で入力して下さい。");
			return false;
		}
}
...
<%
	...
	If Not rsd.EOF Then
		If Trim(rsd("ContNo")) <> "" Then		  'コンテナ番号
			sContNo = Trim(rsd("ContNo"))
		Else
			sContNo = Trim(rsd("BLNo"))
		End If
		If Trim(rsd("RecDel")) = "R" Then		  '入／出区分
			sRecDel = "搬入"
		Else
			sRecDel = "搬出"
		End If
...
<%		Else			%>
	<tr ALIGN=middle><td width="100" bgcolor ="#000080"><FONT COLOR="#ffffff">搬入元変更</td></tr>
<%		End If	%>
	<td></td>
