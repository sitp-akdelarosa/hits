...
'			 strWhere & strOrder1		
	...
	StrSQL = "SELECT ISNULL(CONVERT(varchar(10),SPB.InputDate,111),'') as InputDate,"&_
			 ...
			 " LEFT JOIN Booking As BOK ON SPB.BookNo = BOK.BookNo"&_
			 " LEFT JOIN mUsers AS mU ON SPB.SenderCode = mU.UserCode"&_
			 " LEFT JOIN ViewExportCnt As VEC ON BOK.VslCode = VEC.VslCode AND BOK.VoyCtrl = VEC.VoyCtrl AND BOK.BookNo = VEC.BookNo"&_
			 " LEFT JOIN mVessel AS mV ON BOK.VslCode = mV.VslCode"&_
...
function displayColumn(str,TableName){
	...
	if (TableName=='TBEmpty'){
		...
		for(var intRowCnt=1; intRowCnt<intMaxRow; intRowCnt++) {
			...
			for(var intColCnt=0; intColCnt<17; intColCnt++) { //17->18
				//ヘッダーIDを設定
				if(intColCnt<9){
	...
	<tr>
		<td>
			<div id="BDIV2">				
				<% If Num2>0 Then%>
					<% If blnSorted2 Then%>
					<iframe frameborder="0" style="background-color:#00FF00;position:absolute;top:expression(this.offsetParent.scrollTop);left:0px;width:400;height:47px;"></iframe>				
					<% Else%>
					<iframe frameborder="0" style="background-color:#00FF00;position:absolute;top:expression(this.offsetParent.scrollTop);left:0px;width:400;height:37px;"></iframe>									
					<% End If%>
					<table border="1" cellpadding="0" cellspacing="0" width="100%" Id="TBInOut">				
						<thead>
							<tr>
								...
								<th id="H2Col16" class="hlist" nowrap><%=HeaderTbl2(15)%></th>
								<th id="H2Col17" class="hlist" nowrap><%=HeaderTbl2(16)%></th>						
						...
						<tbody>
							...
							<% end if%>																	
								...
								<% else %>
								<td id="D2Col07" nowrap><A HREF="JavaScript:GoRenew2('<%=ObjRS2("WkNo")%>','<%=ObjRS2("BookNo")%>','<%=ObjRS2("ContNo")%>');"><%=Trim(ObjRS2("WkNo"))%></A><BR></td>																
								<% end if %>
								<% if Trim(ObjRS2("Type")) = "FULLOUT" then%>						
								...
								<% else %>
								<td id="Td1" nowrap><A HREF="JavaScript:GoConinfContNo('<%=Trim(ObjRS2("BLContNo"))%>');"><%=Trim(ObjRS2("BLContNo"))%></A><BR></td>																
								<% end if %>
								...
								<!--2010/05/12 Add-S C.Pestano-->
								<td id="D2Col16" nowrap><%=Trim(ObjRS2("Code2"))%><BR></td>
								<td id="D2Col17" nowrap><%=Trim(ObjRS2("Flag1"))%><BR></td>
								<!--2010/05/12 Add-E C.Pestano-->
