...
'データ取得
  ...
  FieldName=Array("InPutDate","Code1","BookNo","NumC","ContSize1","ContType1","ContHeight1","ContMaterial1","ShipLine","FullName","CYCut","PickPlace","TruckerCode","TruckerFlag","Comment1","Comment2","TruckerSubName")
...
'  DtTbl(0)=Array("入力日","指示元","ブッキング番号","ピック済本数","指示先","指示先回答","照会フラグ","作業完了F","船社","船名","指示元表示用","備考１","指示先データ","備考２","サイズ","タイプ","高さ","材質")
  ...
  DtTbl(0)=Array("入力日","指示元","ブッキング番号","ピック<BR>済","指示先","指示先<BR>回答","照会フラグ","作業完了F","船社","船名","指示元表示用","備考１","指示先データ","備考２","SZ","タイプ","高さ","材質","CYカット日","空コン搬出先","コード","担当","コード","担当")
'Chang 20050303 END
...
'2010/04/24 M.Marquez Del-E
        ...
        StrSQL = StrSQL & "            SPB.SenderCode, SPB.InputDate, SPB.TruckerCode, SPB.TruckerFlag, "
	...
	'2010/04/25 Upd-S Tanaka
        'StrSQL = StrSQL & "           EXC.NumC, "
	...
	'2010/04/25 Upd-E Tanaka
        ...
        StrSQL = StrSQL & "            VSLS.CYCut "
        if Session("Key1") = "" AND Session("Key2") = "" AND Session("Key3") = "" then
            Session("Key1")=0
            Session("KeySort1")="ASC"
        end if
        if Session("Key1") <> "" Then 
            if FieldName(Session("Key1"))="Code1" Then 
                StrSQL = StrSQL & ",(CASE WHEN mU.UserType = '5' THEN mU.HeadCompanyCode ELSE SPB.SenderCode END) as Code1Ord "
            elseif FieldName(Session("Key1"))="BookNo" Then 
                StrSQL = StrSQL & ",SPB.BookNo as BookNoOrd "
            elseif FieldName(Session("Key1"))="ShipLine" Then 
                StrSQL = StrSQL & ",SPB.ShipLine as ShipLineOrd "
            else
                StrSQL = StrSQL & "," & ChangeSortItem1(FieldName(Session("Key1")))
            end if            
        end if
        if Session("Key2") <> "" Then 
            if FieldName(Session("Key2"))="Code1" Then 
                StrSQL = StrSQL & ",(CASE WHEN mU.UserType = '5' THEN mU.HeadCompanyCode ELSE SPB.SenderCode END) as Code1Ord "
            elseif FieldName(Session("Key2"))="BookNo" Then 
                StrSQL = StrSQL & ",SPB.BookNo as BookNoOrd "
            elseif FieldName(Session("Key2"))="ShipLine" Then 
                StrSQL = StrSQL & ",SPB.ShipLine as ShipLineOrd "
            else
                StrSQL = StrSQL & "," & ChangeSortItem1(FieldName(Session("Key2")))
            end if
        end if
        if Session("Key3") <> "" Then 
            if FieldName(Session("Key3"))="Code1" Then 
                StrSQL = StrSQL & ",(CASE WHEN mU.UserType = '5' THEN mU.HeadCompanyCode ELSE SPB.SenderCode END) as Code1Ord "
            elseif FieldName(Session("Key3"))="BookNo" Then 
                StrSQL = StrSQL & ",SPB.BookNo as BookNoOrd "
            elseif FieldName(Session("Key3"))="ShipLine" Then 
                StrSQL = StrSQL & ",SPB.ShipLine as ShipLineOrd "                
            else            
                StrSQL = StrSQL & "," & ChangeSortItem1(FieldName(Session("Key3")))
            end if
        end if
        ...
        if Session("Key1") <> "" Then 
            StrSQL = StrSQL & FieldName(Session("Key1")) & "Ord ASC," & FieldName(Session("Key1")) & " " & Session("KeySort1") & " "
        end if
        if Session("Key2") <> "" Then 
            if Session("Key1") <> "" Then 
                StrSQL = StrSQL & ","
            end if
            StrSQL = StrSQL & FieldName(Session("Key2")) & "Ord ASC," & FieldName(Session("Key2")) & " "  & Session("KeySort2") & " "
        end if
        if Session("Key3") <> "" Then 
            if Session("Key1") <> "" OR Session("Key2") <> ""Then 
                StrSQL = StrSQL & ","
            end if

            StrSQL = StrSQL & FieldName(Session("Key3")) & "Ord ASC," & FieldName(Session("Key3")) & " " & Session("KeySort3") & " "
        end if    
             	'strOrder
