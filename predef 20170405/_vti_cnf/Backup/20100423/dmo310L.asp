...
'  DtTbl(0)=Array("入力日","搬入<BR>予定日","指示元","作業番号","ブッキング番号","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元へ回答","船社","船名","SZ","H","搬入元","CY","CYカット日","備考１","備考２","備考３","備考４","TW")
  DtTbl(0)=Array("入力日","搬入<BR>予定日","指示元<BR>コード","作業<BR>番号","ブッキング番号","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元へ回答","船社","船名","SZ","H","搬入元","CY","CY<BR>カット日","備考１","備考２","備考３","備考４","TW","コード","担当")
...
'3th ADD LINE:"INNER JOIN CYVanInfo AS CYV ON ITC.WkNo = CYV.WkNo AND ITC.ContNo=CYV.ContNo "&_
  ...
  'データ取得
    StrSQL = "SELECT T.* FROM (SELECT ITC.InputDate, ITC.WorkDate, ITC.WkNo, ITC.WorkCompleteDate, ITC.ContNo, ITC.RegisterCode, "&_
             ...
             "CYV.BookNo, CYV.ShipLine, CYV.VslName, CYV.ContSize, CYV.ContHeight, CYV.ReceiveFrom, CYV.TareWeight, "&_
...
<%If Num<>0 Then%>
    ...
    if((document.body.offsetWidth-10)  < 1481){
...
function GoCSV(){
  target=document.dmo310F;
//2009/10/02 Add-S Fujiyama 確認メッセージ
 flag = ShowMessage('最新情報を取得済ですか？,,＊「表示種類切替→出力したい項目」を再クリックすると最新データが取得されますので、,その操作を行った後に保存して下さい。');
 if(flag==true){
...
<!-- C-001    <TD nowrap><%=DtTbl(j)(11)%></TD><TD nowrap><%=DtTbl(j)(12)%></TD><TD nowrap><%=DtTbl(j)(13)%></TD> -->
  ...
  <TR class=bga>
    ...
    <TH class="hlist" nowrap><%=DtTbl(0)(3)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(4)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(5)%></TH>
    <TH class="hlist" nowrap><%=DtTbl(0)(11)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(12)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(13)%></TH>
    <TH class="hlist" nowrap><%=DtTbl(0)(14)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(22)%></TH><TH <TH class="hlist" nowrap><%=DtTbl(0)(15)%></TH>
    <TH class="hlist" nowrap><%=DtTbl(0)(16)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(17)%></TH>
    <TH class="hlist" nowrap><%=DtTbl(0)(6)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(7)%></TH><TH <TH class="hlist" nowrap><%=DtTbl(0)(8)%></TH>
    <TH class="hlist" nowrap><%=DtTbl(0)(18)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(19)%></TH><TH class="hlist" nowrap><%=DtTbl(0)(20)%></TH>
    <TH class="hlist" nowrap><%=DtTbl(0)(24)%></TH>
    <INPUT type=hidden name='Datatbl0' disabled value='<%=DtTbl(0)(0)%>,<%=DtTbl(0)(1)%>,<%=DtTbl(0)(2)%>,<%=DtTbl(0)(3)%>,<%=DtTbl(0)(4)%>,<%=DtTbl(0)(5)%>,<%=DtTbl(0)(6)%>,<%=DtTbl(0)(7)%>,<%=DtTbl(0)(8)%>,<%=DtTbl(0)(9)%>,<%=DtTbl(0)(10)%>,<%=DtTbl(0)(11)%>,<%=DtTbl(0)(12)%>,<%=DtTbl(0)(13)%>,<%=DtTbl(0)(14)%>,<%=DtTbl(0)(15)%>,<%=DtTbl(0)(16)%>,<%=DtTbl(0)(17)%>,<%=DtTbl(0)(18)%>,<%=DtTbl(0)(19)%>,<%=DtTbl(0)(20)%>,<%=DtTbl(0)(21)%>,<%=DtTbl(0)(22)%>,<%=DtTbl(0)(23)%>,<%=DtTbl(0)(24)%>'>
  </TH></TR>
