...
'2010/5/15 MES Aoyagi Mod-E
		...
		'2016/08/23 H.Yoshikawa Add Start
        StrSQL = StrSQL & "INNER JOIN (select BookNo, SenderCode, TruckerCode, Min(Seq) AS Seq, Max(OutFlag) as OutMax, Min(OutFlag) as OutMin From BookingAssign Group BY BookNo, SenderCode, TruckerCode) BA2"
	...
	if(IEVersion < 10)
	{
      if((document.body.offsetWidth-10) <= 1077){
		...
		else
		{
			...
			{
				obj3.style.width=document.body.offsetWidth-(document.body.offsetWidth-1160);
				obj3.style.overflowX="auto";
			}
			else
			{
