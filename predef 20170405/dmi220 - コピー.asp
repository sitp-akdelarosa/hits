...
<% For i=0 To 4%>
	...
	<% '2016/08/22 H.Yoshikawa Add End %>
      ...
      <!-- 2016/08/22 H.Yoshikawa Upd Start -->
          ...
          <TD><INPUT type=text name="PickDate<%=i%>"  value="<%=Request("PickDate"&i)%>" size=15 <%=Dflag2%> maxlength=10>
              <INPUT type=hidden name="Bef_PickDate<%=i%>" value="<%=Request("Bef_PickDate"&i)%>">
              <INPUT type=text name="PickHour<%=i%>"  value="<%=Request("PickHour"&i)%>" size=4 <%=Dflag2%> maxlength=2>時
              <INPUT type=text name="PickMinute<%=i%>"  value="<%=Request("PickMinute"&i)%>" size=4 <%=Dflag2%> maxlength=2>分
