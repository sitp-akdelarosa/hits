...
' 集計ロジック
	...
	'' ログファイルの取得
	Dim strFileName
	strFileName="./log/" & strDateTime & ".log"
	...
	'カメリア対応追加↓↓↓↓
	strFileName="./cam/log/" & strDateTime & ".log"
	...
	If LineNo>0 Then
		...
		For i=0 to iTKind-1
			...
			For j=0 to LineNo-1
				...
				If anyTmp(1)=PageNum(i) and anyTmp(3)=WkNum(i) then
					if anyTmp(5)<>"90000" and anyTmp(5)<>"10000" and anyTmp(5)<>"MES" and anyTmp(5)<>"MEO" Then
						Count(i)=Count(i)+1
					End If
				End If
			Next
...
<tr>
	<td valign="top">
		...
		<table border="0">
		<tr><td align="left">
			<form method="post" name="input" action="logview.asp?kind=1">
