...
<%
    ...
    Else
        Session.Contents("findkind")="Blno"
    End If
...
'    fpath=fs.GetFileName(filename)
    ...
    If lng<=0 Then
        bError=true
        strError = "'" & filename & "' Failed in transferring the file."
    Else
        Dim strCntnrNo()
    ...
    Else
        strOption = filename & "," & "入力内容の正誤:0(正しい)"
    End If

    ' 輸入コンテナ照会
    WriteLog fs, "2303","輸入コンテナ照会-CSVファイル転送",iWrkNum, strOption
...
End of comment by seiko-denki 2003.07.18 -->
      ...
      <table>
        <tr> 
          <td nowrap>
            <font color="#000066" size="+1">[for import container ]</font>
			<BR><br>
<%
    ' エラーメッセージの表示
