...
' Added and Commented by seiko-denki 2003.07.07
	'Y.TAKAKUWA DEL-S 2015-02-23
	'DisplayCodeListButton
	'Y.TAKAKUWA DEL-E 2015-02-23
'    DispMenu
...
<!-- 2010.5.19 Mod-S MES Aoyagi -->
	  ...
	  <%="Please input the company ID No. and password and click 'SEND'."%>

<!-- 2010.5.19 Mod-E MES Aoyagi -->
              <center>
              <table border="1" cellspacing="2" cellpadding="3" bgcolor="#ffffff">
                <tr> 
                  <td bgcolor="#ff8833" nowrap align=center valign=middle> <font color="#FFFFFF"><b>COMPANY ID</b></font></td>
                  <td nowrap>
              ...
              <br>
                <input type="submit" value=" SEND " onClick="return Check()"></center>
              </form>
...
%>
	  ...
	  <!--2015-02-23 Y.TAKAKUWA Upd-S-->
	  <!--<td align="Left"><b><font color="Blue"><%= "　操作しない状態が" & Session.Timeout & "分以上続くと自動的にログアウトします。<br> &nbsp;&nbsp;その場合、お手数ですが再度ログインのうえご利用ください。"%></Font></b></td>-->
	  <td align="Left"><b><font color="Blue"><%= "It will be automatically logout without using the system for " & Session.Timeout & " min. <br> &nbsp;&nbsp;In this case, please login again to use the system."%></Font></b></td>
	  <!--2015-02-23 Y.TAKAKUWA Upd-E-->
	  <!--2015-02-23 Y.TAKAKUWA DEL-S-->
	  <!--<td align="Left"><b><font color="Blue"><%= "&nbsp;&nbsp;トップページに'ログイン中'が表示されていてもタイムアウト<br>&nbsp;&nbsp;(約" & Session.Timeout & "分）により本画面が表示される可能性があります。"%></font></b></td>-->
	  <!--2015-02-23 Y.TAKAKUWA DEL-E-->
	  <!--2011/11/16 M.Marquez Upd-E-->
	  <td></td>
	</tr>
        <tr>
                  <td><img src="gif/bo-yellow.gif" width="18" height="18"></td>
                  <td align="right"><img src="gif/1.gif" width=1 height=1><a href="javascript:OpenWindow()" ><font size=2 color="#0000ff">Information Update for Users</font></a></td>
                  <td><img src="gif/bo-yellow.gif" width="18" height="18"></td>
	          </td>
             </tr>
           </table>
...
<br><br><br>
<a href="touroku/index.html" target="new">COMPANY ID REGISTRATION</a>
  </center>
    </td>
 </tr>
 <tr>
...
<%
    ...
    Select Case strLinkID
        Case "hits.asp", "gate.asp"
			...
			if strLinkID = "index_en.asp" then
			  strLinkID = "/" + strLinkID
			ElseIf strLinkID = "predef/dmi000F.asp" then
			  strLinkID = "/" + strLinkID
			end if
...
'				iWrkNum = 13  ' End of Comment by seiko-denki 2003.07.07
		...
		Case "pickselect.asp"
				iNum = "a100"
				iWrkNum = 10
		Case "predef/dmi000F.asp","index.asp","index_en.asp"
				iNum = "b000"
				iWrkNum = 10
		...
		Case Else
				' 戻り画面へリダイレクト
 				CheckLinkLog
				'Y.TAKAKUWA Add-S 2015-03-03			
			    if strLinkID = "index_en.asp" then
			      strLinkID = "/" + strLinkID
			    ElseIf strLinkID = "predef/dmi000F.asp" then
			      strLinkID = "/" + strLinkID
			    end if
			    'Y.TAKAKUWA Add-E 2015-03-03
				Response.Redirect strLinkID
				Response.End
	End Select
	...
	if strLinkID = "index.asp" or strLinkID = "index_en.asp" then
		strUserID = ""				
	end if
...
if Request.QueryString("Screen_Mode") <> "R" then '2009/02/25 Add G.Ariola  
            If bOK=false Then
                ' ユーザーＩＤエラーのとき
                bError=true
                strError = "Incorrect Data Input." '"入力された内容に間違いがあります。"
                ' ログインエラー回数をカウントアップ
                iError=iError+1
                Session.Contents("loginerror") = iError
            End If
