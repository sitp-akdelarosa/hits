...
function SearchContBL(){
	...
	if(!f.ContORBL[0].checked && !f.ContORBL[1].checked){
		alert("Choose the  search item");
		return false;
	}
	Num=LTrim(f.ContBLNo.value);
	if(Num.length==0){
		alert("Input the No. to search");
		f.ContBLNo.focus();
		return false;
	}
...
<TABLE border="0" cellPadding="3" cellSpacing="0" width="100%">
	...
	<TR>
		<TD width="50%"><BR><B class=title>IMPORT STATUS DELIVERY REQUEST LIST</B></TD>
		<TD width="50%">
			<INPUT type="hidden" name="ContBLFlag" value="">
			<INPUT type="radio" name="ContORBL">Container No. 
			<INPUT type="radio" name="ContORBL">BL No.<BR>
			<INPUT type="text"  name="ContBLNo" maxlength="20">
			<INPUT type="button" value="SEARCH" onClick="SearchContBL()">
		</TD>
	<TR>
	</FORM>
</TABLE>
