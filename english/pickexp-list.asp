...
<% Else %>
				<td nowrap align=center valign=middle><%=anyTmp(15)%></td>
<% End If %>

			<form method=post action="pickexp-input.asp?kind=0">
				<td nowrap align=center valign=middle>
					...
					<input type=hidden name="receivedate"	 value="<%=anyTmp(13)%>">
					<input type=hidden name="opecode"	 value="<%=anyTmp(14)%>">
					<input type=hidden name="forwarder"	 value="<%=anyTmp(15)%>">
...
<% If strUserKind="海貨" Then %>
				<td nowrap align=center valign=middle><%=anyTmp(14)%></td>
...
<%
		...
		End If
		If Not anyTmp(13)="<BR>" Then
			anyTmp(13) = Right(anyTmp(13),5)
		End If
%>
...
<% If strUserKind="海貨" Then %>
				<td nowrap align=center valign=middle><%=anyTmp(13)%></td>
<% End If %>
...
<%
	...
	End If
	' Log作成
    WriteLog fs, iNum,"空コンピックアップシステム-" & strUserKind & "用更新対象一覧", "00", ","
%>
