...
'_/	Modify		:2018/10/16			作業ルール順守チェック追加	_/
'_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
%>
<% Option Explicit %>
<%
...
'3th  End If
  ...
  dim DefName	'2017/04/04 H.Yoshikawa Add
  ...
  'ヘッドIDのチェック
   ...
   '港コードのチェック
	...
	'確定事業者のチェック
	DefName = ""
	if gfTrim(Request("ReportNo")) <> "" then
		StrSQL = "SELECT DefName From mDefTrade "&_
		       "WHERE DefCode = '"  &gfSQLEncode(Request("ReportNo")) & "'"
		ObjRS.Open StrSQL, ObjConn
		if err <> 0 then
			DisConnDBH ObjConn, ObjRS	'DB切断
			jampErrerP "1","b401","01","実搬入：確定事業者データ取得","103","SQL:<BR>"&StrSQL
		end if
		if ObjRS.EOF then
			ErrMsg = ErrMsg & "入力された届出番号が確定事業者マスタに登録されていません。<BR>"
			ret = false
		else
			DefName = gfTrim(ObjRS("DefName"))
		end if
		ObjRS.Close
	end if
...
function GoEntry(){
  ...
  if(target.ChkRule){
    if(!target.ChkRule.checked){
      alert('「ヤード内注意事項を遵守し、安全作業を行います」にチェックを入れ（誓約して）予約を完了して下さい。了承頂けない場合、作業予約出来ません。');
      return false;
    }
  }
  ...
  <TR>
	...
	<!-- 2016/08/08 H.Yoshikawa Add Start -->
	  ...
	  <!-- 2017/05/08 H.Yoshikawa Del Start
    	...
    	<TD>
    	  <INPUT type=hidden name="EntryName" value="<%=Request("EntryName")%>" >
    	</TD></TR> 2017/05/08 H.Yoshikawa Del End -->
	  <!-- 2017/04/04 H.Yoshikawa Add End -->
	  <TR>
	...
	<TABLE border=0 cellPadding=2 cellSpacing=0 width="100%">
	  <TR>
	    <TD width=300>
		    ...
		    <TR>
		    	...
		    	<TD><INPUT type=text name="SttiT" value="<%=Request("SttiT")%>" size=5>&nbsp;<%=Request("TempDegree")%>
		    	<!--
	  ...
	  <TR>
	    ...
	    <TD>
	        ...
	        <INPUT type=text name="CONtear" value="<%=Request("CONtear")%>" size=9>&nbsp;kg		<!-- 2017/04/20 H.Yoshikawa Upd(size=7→9、kgの前にスペース追加) --> 
	    </TD></TR>
	  <TR>
	  ...
	  <TR>
	    ...
	    <TD><INPUT type=text name="ReportNo" value="<%=Request("ReportNo")%>" size=25 maxlength=18 >　<span style="color:red">※ハイフン付きで入力してください</span></TD></TR>		<!-- 2017/05/07 H.Yoshikawa Upd（size:17⇒25、maxlength:12⇒18、ハイフンなし⇒ハイフン付き） -->
	  <!-- 2017/04/04 H.Yoshikawa Add Start -->
	  <TR>
	    <TD><DIV class=bgb>重量　確定事業者</DIV></TD>
	    <TD><INPUT type=text name="DefName" value="<%=gfHTMLEncode(DefName)%>" size=80></TD></TR>			<!-- 2017/05/10 H.Yoshikawa Upd(size:40⇒80) -->
	  <!-- 2017/04/04 H.Yoshikawa Add End -->
	  <!-- <TR>
	  ...
	  <TR>
	    ...
	    <TD><INPUT type=hidden name="RHO" value="<%=Request("RHO")%>" size=5></TD></TR> <!-- 2017/04/04 H.Yoshikawa Upd text→hidden -->
	  <TR>
		    <TD></TD>
		    <TD>
  ...
  <TABLE border=0 cellPadding=2 cellSpacing=0 width="100%">
	  <TR>
	    <TD colspan=3 align=left valign=bottom>
	    	<INPUT type=checkbox <% if gfTrim(Request("AgreeChk")) = "1" then %>checked<% end if %> disabled><!-- 2017/04/04 H.Yohsikawa Upd 文字色：赤 --><span style="color:red">本画面の入力内容をゲートでの搬入票の代わりとして使用することに同意します。</span>
...
<!--   <div class=observ><INPUT type=checkbox name="ChkRule" value="1" ><a href="../download/download.asp?guide=ヤード内注意事項.pdf">ヤード内注意事項</a>を理解し安全作業を遵守します。</div> -->
       <div class=observ>
			（誓約書）<BR>
			<INPUT type=checkbox name="ChkRule" value="1" ><a href="../download/download.asp?guide=ヤード内注意事項.pdf">「ヤード内注意事項」</a>を遵守し、安全作業を行います。<BR>
			（「ヤード内注意事項」を守らずに起きた損害につきましては、<BR>ターミナルは一切の責任を負いませんのでご了承願います。）
		</div>
