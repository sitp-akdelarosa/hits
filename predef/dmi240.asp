...
'_/	Modify		:03-01　2017/09/27	削除時に搬出可否を確認中に戻す	_/
'_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
%>
<% Option Explicit %>
<%
...
'2006/03/06 mod-e h.matsuda
    ...
    If ret Then
      ...
      '2017/11/01 Mw.Tanaka Add-S   SPBookInfo更新後のの常駐が、BookAsignの更新前に実行されてしまうので、上部に移動
      '入力データ保存
      ret = InsBookAssign()					'2016/08/29 H.Yoshikawa Add
      '2017/11/01 Mw.Tanaka Add-E   
...
'2016/08/29 H.Yoshikawa Del End
      ...
      '2017/11/01 Mw.Tanaka Del-S   SPBookInfo更新後のの常駐が、BookAsignの更新前に実行されてしまうので、上部に移動
      '--		'入力データ保存
      '--		ret = InsBookAssign()					'2016/08/29 H.Yoshikawa Add
      '2017/11/01 Mw.Tanaka Del-E   
    End If
...
'2017/11/01 Mw.Tanaka Add-E   SPBookInfo更新後のの常駐が、BookAsignの更新前に実行されてしまうので、上部に移動
...
'C-002 ADD This Line: ", Comment='"& Request("Comment") &"' "&_
	...
	'--		'2016/08/29 H.Yoshikawa Add End
	'--		
	'2017/11/01 Mw.Tanaka Del-E   SPBookInfo更新後のの常駐が、BookAsignの更新前に実行されてしまうので、上部に移動
...
Function InsBookAssign()
  ...
  dim Qty1
  dim Operator
  dim Seq_str				'2017/05/10 H.Yoshikawa Add
  dim Process				'2017/05/10 H.Yoshikawa Add
		...
		if gfTrim(Request("ContSize" & i)) <> "" then
			...
			if Request("Mord") = 1 Then				'更新モード
				...
				Do Until oldObjRS.eof
					...
					elseif gfTrim(oldObjRS("Seq")) = "A" then
						Seq = 10
			...
			if gfTrim(Request("DelFlag" & i)) = "1" then
				Process = "D"
				OutFlag = "0"		'2017/09/27 03-01 Add Fujiyama
			else
				Process = "R"
			end if
			'2017/05/10 H.Yoshikawa Add End
	        ...
	        StrSQL =  StrSQL & ", '" & Seq_str & "' "							'2017/05/10 H.Yoshikawa Upd(Seq⇒Seq_str)
	        StrSQL =  StrSQL & ",'" &  Now() & "' "
	        StrSQL =  StrSQL & ",'PREDEF01' "
	        StrSQL =  StrSQL & ",'" & gfSQLEncode(USER) & "' "
	        StrSQL =  StrSQL & ",'"	& Process & "' "							'2017/05/10 H.Yoshikawa Upd('R'⇒Process)
	        StrSQL =  StrSQL & ",'" & Now() & "' "
	        ...
	        if gfTrim(Request("SetTemp" & i)) = "" then
	        	...
	        	StrSQL =  StrSQL & ",'" & gfSQLEncode(Replace(Request("SetTemp" & i), "+", "")) & "C' "			'2017/05/10 H.Yoshikawa Upd ("+"を取る)
	        end if
