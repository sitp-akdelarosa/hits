...
'_/	Modify		:20201201 h.matsuda 作業終了分は「削除」ボタン非表示、「更新」ボタンに習う_/
'_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
%>
<% Option Explicit %>
<%
...
'2016/11/08 H.Yoshikawa Add Start
  if Operator <> "" then
	...
	dim TempDegree
	'2017/07/31 H.Yoshikawa Upd Start 作業番号指定時もBookContから取得
	'ExcSetTemp = Request("SttiT")
	ExcSetTemp = ""
	'2017/07/31 H.Yoshikawa Upd End
	...
	if ExcType = "RF" and ExcSetTemp = "" then
		...
		if ExcSetTemp = "" then
			'2017/07/31 H.Yoshikawa Add Start ブッキングコンテナにない場合、ブッキングから取得
			StrSQL1 = "SELECT ob.SetTemp FROM " & Operator & "_oBook ob "
			StrSQL1 = StrSQL1 & " INNER JOIN KAC_kMVessel mv on mv.VslCode = ob.VslCode "
			StrSQL1 = StrSQL1 & " INNER JOIN KAC_kSchedule kc on kc.VslCode = ob.VslCode and kc.Voyage = ob.Voyage "
			StrSQL1 = StrSQL1 & " WHERE ob.BookNo='"& gfSQLEncode(BookNo) &"' "
			StrSQL1 = StrSQL1 & "   AND mv.CallSign='"& gfSQLEncode(VslCode) &"' "
		...
		'2017/07/31 H.Yoshikawa Add End
	end if
...
//2017/06/19 H.Yoshikawa Upd Start
  ret = CheckSu(target.CONtear.value); 
  if(ret==false){
    alert("テアウェイトに半角数字以外を入力しないでください");
    target.CONtear.focus();
    return false;
  }
//2017/06/19 H.Yoshikawa Upd End
...
// ret = CheckSu(Num)
//2017/05/07 H.Yoshikawa Upd Start
// ret = CheckEisu2(Num)
 ret = CheckEisu3(Num, "-")
//2017/05/07 H.Yoshikawa Upd End
 if(!ret){
   //alert("届出番号または登録番号は数値で入力してください。");
   //2017/05/07 H.Yoshikawa Upd Start
   //alert("届出番号または登録番号は半角英数字で入力してください。");
   alert("届出番号または登録番号は半角英数字またはハイフンのみで入力してください。");
   //2017/05/07 H.Yoshikawa Upd End
   strA[7].focus();
   return false;
 }
...
// if(Num.length!=0 && Num.length!=18){	//2017/04/05 H.Yoshikawa Upd（12桁⇒18桁）
//   alert("届出番号または登録番号は18桁で入力してください。");
//   strA[7].focus();
//   return false;
// }
...
//2017/06/26 H.Yoshikawa Add Del
//  //2017/06/16 H.Yoshikawa Add Start
//  //RHOに「O」がない場合、ワーニング
//  errmsg = "ターミナルが把握していないオーバーゲージ貨物になります。搬入前にオペレータに連絡してください。";
//  if(target.RHO.value.indexOf("O") < 0){
//  	if(Number(target.OH.value) > 0){
//		alert(errmsg);
//  	}
//  	if(Number(target.OWL.value) > 0){
...
//  	}
//  }
//  //2017/06/16 H.Yoshikawa Add End
//2017/06/26 H.Yoshikawa Add End
  
  //2017/06/26 H.Yoshikawa Add Start
  //OD入力有の場合、oContainer、oBookQtyのタイプにOT,FR,SOのいずれかがなければエラーとする
  if(Number(target.OH.value) > 0 || Number(target.OWL.value) > 0 || Number(target.OWR.value) > 0
    || Number(target.OLF.value) > 0 || Number(target.OLA.value) > 0){
		retflg = false;
        if("<%=ExcType %>" == "OT" || "<%=ExcType %>" == "FR" || "<%=ExcType %>" == "SO"){
        	retflg = true;
        }
        else
		{
			<% for i = 0 to UBound(ExcSizeAry) %>
				if("<%=ExcTypeAry(i) %>" == "OT" || "<%=ExcTypeAry(i) %>" == "FR" || "<%=ExcTypeAry(i) %>" == "SO"){
					retflg = true;
				}
			<% next %>
   		}
		...
		if(retflg == false){
			alert("コンテナ規格外貨物情報に誤りがあります。オペレータに連絡してください。");
			return false;
		}
  }
  //2017/06/26 H.Yoshikawa Add End
...
<TD width=300 valign=top>
	<TABLE border=0 cellPadding=2 cellSpacing=0 width="100%">
	  ...
	  <TR>
	    <TD style="padding-bottom:0px;" nowrap><DIV class=bgb>*コンテナ総重量</DIV></TD>
	  ...
	  <TR>
	    ...
	    <TD><INPUT type=text name="ReportNo" value="<%=Request("ReportNo")%>" size=28 maxlength=18 title="荷送人の届出番号、または登録確定事業者番号を入力してください。">		<!-- 2017/05/07 H.Yoshikawa Upd（size:17⇒28、maxlength:12⇒18） -->
	        <INPUT type=button value="検索" onClick="DfTSelect();">		<!-- 2017/04/04 H.Yoshikawa Add -->
	        　<span style="color:red">※ハイフン付きで入力してください</span></TD></TR>		<!-- 2017/05/07 H.Yoshikawa Upd（ハイフンなし⇒ハイフン付き） -->
	  <!-- 2017/04/04 H.Yoshikawa Add Start -->
	  <TR>
	    <TD nowrap><DIV class=bgb>重量　確定事業者</DIV></TD>
	    <TD><INPUT type=text name="DefName" value="<%=Request("DefName")%>" size=80 readOnly></TD></TR>					<!-- 2017/05/10 H.Yoshikawa Upd(size:40⇒80) -->
	  <!-- 2017/04/04 H.Yoshikawa Add End -->
	  ...
	  <!-- 2016/08/01 H.Yoshikawa Add Start -->
	  <TR>
	    <TD style="padding-top:0px;" nowrap><DIV class=bgb>少量危険品</DIV></TD>
	    <TD style="padding-top:0px;">
	        <TABLE cellpadding=0 cellspacing=0><TR>
	...
	<% Else %>
	  ...
	  '   IF Request("TruckerFlag")<>1 AND Request("compFlag") Then 
	     IF Request("compFlag") Then %>
		  ...
		  <!--End If 2020/12/01 h.matsuda move from -->
	      ...
	      <% End If %>
	  <% End If %><!--2020/12/01 h.matsuda move to 終了分は、削除不可-->
	<% End If %>
