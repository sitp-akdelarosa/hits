...
'3th Add End
	...
	StrSQL = "SELECT T.* FROM (SELECT ITC.InputDate, ITC.WorkDate, ITC.RegisterCode, ITC.TruckerSubCode1, ITC.TruckerSubCode2, "&_
      ...
      if Trim(Session("Key1"))<>"" Then 
            StrSQL = StrSQL & "," & ChangeSortItem1(Session("Key1"))
      end if
      if Trim(Session("Key2"))<>"" Then 
            StrSQL = StrSQL & "," & ChangeSortItem1(Session("Key2"))
      end if
      if Trim(Session("Key3"))<>"" Then 
            StrSQL = StrSQL & "," & ChangeSortItem1(Session("Key3"))
      end if
	  ...
	  StrSQL = StrSQL &	"FROM ((((hITCommonInfo AS ITC INNER JOIN hITReference AS ITR ON ITC.WkContrlNo = ITR.WkContrlNo) "&_
		...
		"WHERE WkType='2' AND (RegisterCode='"& USER &"' "&_
		"OR TruckerSubCode1='"& COMPcd &"' OR TruckerSubCode2='"&_
		COMPcd &"' OR TruckerSubCode3='"& COMPcd &"' OR TruckerSubCode4='"& COMPcd &"') AND Process='R' "& strWrer &") AS T "
      
      if Trim(Session("Key1"))<>"" OR Trim(Session("Key2"))<>"" OR Trim(Session("Key3"))<>"" Then                
        StrSQL = StrSQL & " ORDER BY "
      end if
      if Trim(Session("Key1"))<>"" Then 
            StrSQL = StrSQL & (Session("Key1")) & "Ord " (Session("KeySort1"))
      end if
      if Trim(Session("Key2"))<>"" Then 
            StrSQL = StrSQL & (Session("Key2")) & "Ord " (Session("KeySort2"))
      end if
      if Trim(Session("Key3"))<>"" Then 
            StrSQL = StrSQL & (Session("Key3")) & "Ord " (Session("KeySort3"))
      end if    
...
<BODY leftMargin=0 topMargin=0 marginheight="0" marginwidth="0" onLoad="setTimeout('showContent()', 500);vew();" onResize="vew()" onBlur="vew()">
<!-------------空搬入情報一覧画面List--------------------------->
