...
'_/	Modify		:	20170206 T.Okui 全体レイアウト変更		     _/
'_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
%>
<% Option Explicit %>
<%
...
'2017/04/04 H.Yoshikawa Upd(確定事業者マスタmDefTradeをひもづけし、事業者名を取得。また、指示元担当者の値を取得。)
  StrSQL = "SELECT CYV.ShipLine, CYV.VslName, CYV.Voyage, CYV.DPort, CYV.DelivPlace, CYV.ContSize, CYV.ContType, "&_
           ...
           ",BOK.Sender ,CYV.EntryName ,mDT.DefName "&_
           ...
           "LEFT JOIN mDefTrade AS mDT ON mDT.DefCode = CYV.ReportNo "&_
...
'oContainerからテアウェイト取得するように変更
'2017/03/03 T.Okui 元に戻す	    
		'oContainerからコンテナタイプ取得
		StrSQL = "SELECT oc.ContType FROM " & Trim(Operator) & "_oContainer oc "
		'StrSQL = "SELECT oc.ContType,oc.TareWeight FROM " & Trim(Operator) & "_oContainer oc "
		...
		if not ObjRS5.EOF then
			...
			'TareWeight    = gfTrim(ObjRS5("TareWeight"))  '2017/03/02 T.Okui Add
		end if
	  ...
	  <TD>
    	...
    	<!-- 2016/08/17 H.Yoshikawa Add Start -->
    	  ...
    	  <TR>
    	    ...
    	    -->
    	    <TD><INPUT type=text name="OpeName" value="<%=gfConvertOperator(Trim(ObjRS("Operator")))%>"></TD>
    	    <INPUT type=hidden name="Operator" value="<%=Trim(ObjRS("Operator")) %>">
    	    </TR>
    	    <!-- 2017/03/02 T.Okui Upd-E -->
		</TABLE>
		</DIV>
			...
			<TD>
				<DIV style="height:160px;width:500px;border: 1px solid black; margin:5px;">
					...
					<TR>
						...
						<!-- 2017/05/08 H.Yoshikawa Del Start
						<TR>
							...
							<!-- 2017/04/04 H.Yoshikawa Upd Start
								...
								<INPUT type=text name="EntryName"  value="<%=gfHTMLEncode(gfTrim(ObjRS("EntryName")))%>" maxlength=16> -->
							<!-- 2017/04/04 H.Yoshikawa Upd End -->
						<!-- </TD>
						</TR> 2017/05/08 H.Yoshikawa Del End -->
						<!-- 2009/10/09 Add-S Fujiyama -->
	  ...
	  <TABLE border=0 cellPadding=2 cellSpacing=0 width="100%">
	    ...
	    <TD colspan=2><DIV><SPAN  STYLE="FONT-WEIGHT:BOLD;">搬入票署名欄情報</SPAN><SPAN STYLE="color:red;">※この欄が搬入票署名欄に印字されます</SPAN></DIV></TD>
	  	...
	  	<TR><TD><DIV class=bgb>*取扱海貨社名</DIV></TD>
	    	<TD><INPUT type=text name="Forwarder" value="<%=gfTrim(ObjRS("PRForwarder"))%>" style="margin-bottom:2px;" size=40></TD></TR>
	    <TR>
	    	<TD><DIV class=bgb>*（担当者）</DIV></TD>
	    	<TD><INPUT type=text name="FwdStaff" value="<%=gfTrim(ObjRS("PRForwarderTan"))%>" ></TD></TR>
	    <TR>
	    	<TD><DIV class=bgb>*（連絡先）</DIV></TD>
	    	<TD><INPUT type=text name="FwdTel" value="<%=gfTrim(ObjRS("PRForwarderTel"))%>" ></TD></TR>
	  	</TR>
	</TABLE>
	</DIV>
	</TD>
	</TR>
	<!-- 2017/06/16 H.Yoshikawa Del Start
	<TR>
	  <TD>
...
<!-- 2016/08/18 H.Yoshikawa Add-S -->
	...
	<!-- 2017/06/16 H.Yoshikawa Del End -->
	...
	<TABLE border=0 cellPadding=2 cellSpacing=0 width="100%">
	    ...
	    <TD valign=top>
	     	<DIV style="height:150px;width:300px;border: 1px solid black; margin:5px;">
		  	<TABLE cellpadding=1 cellspacing=0>
			    ...
			    <TD><INPUT type=text name="OH"  value="<%=Trim(ObjRS("OvHeight"))%>"  size=5 >&nbsp;cm</TD></TR>
			    <TR>
			    <TD><DIV class=bgb>オーバーワイド（右）</DIV></TD>
			    <TD><INPUT type=text name="OWR" value="<%=Trim(ObjRS("OvWidthR"))%>" size=5 >&nbsp;cm</TD></TR>
			    <TR>
			    <TD><DIV class=bgb>オーバーワイド（左）</DIV></TD>
			    <TD><INPUT type=text name="OWL" value="<%=Trim(ObjRS("OvWidthL"))%>" size=5 >&nbsp;cm</TD></TR>
			    <TR>
			    <TR>
			    <TD><DIV class=bgb>オーバーレンクス（前）</DIV></TD>													<!-- 2017/04/20 H.Yoshikawa Upd(レングス→レンクス) --> 
			    <TD><INPUT type=text name="OLF" value="<%=Trim(ObjRS("OvLengthF"))%>" size=5 >&nbsp;cm</TD></TR>
			    <TR>
			    <TD><DIV class=bgb>オーバーレンクス（後）</DIV></TD>													<!-- 2017/04/20 H.Yoshikawa Upd(レングス→レンクス) --> 
			    <TD><INPUT type=text name="OLA" value="<%=Trim(ObjRS("OvLengthA"))%>" size=5 >&nbsp;cm</TD></TR>
			</TR></TABLE></DIV>
	    </TD>
	  </TR>  
	  <TR>
		<TD colspan=2>
			<DIV style="border: 1px solid black; margin:5px;height:145px;">
				<TABLE>
					<TR>
						<TD style="width:50%;font-weight:bold" nowrap>重量情報</TD>
						<TD style="width:50%;"></TD>
					</TR>
					<TR>
						<TD nowrap><DIV class=bgb>コンテナ総重量</DIV></TD>
						<TD><INPUT type=text name="GrosW" value="<%=Trim(ObjRS("ContWeight"))%>" size=9>&nbsp;kg</TD>
					</TR>			<!-- 2017/04/20 H.Yoshikawa Upd(size=5→9) -->
					<TR>
						<TD nowrap><DIV class=bgb>テアウェイト</DIV></TD>
						<TD><INPUT type=text name="CONtear" value="<%=Trim(ObjRS("TareWeight"))%>" size=9>&nbsp;kg</TD>
					</TR>		<!-- 2017/04/20 H.Yoshikawa Upd(size=7→9) -->
					
					<!-- 2016/08/08 H.Yoshikawa Add Start -->
					<TR>
						<TD nowrap><DIV class=bgb>計量方法（確認）</DIV></TD>
						<TD style="padding-top:0px;">
							<INPUT type=hidden name="SolasChk" value="<%=gfTrim(ObjRS("Solas"))%>" >
							<INPUT type=checkbox <% if gfTrim(ObjRS("Solas")) = "1" then %>checked<% end if %> disabled>ここに入力したコンテナ総重量はSOLAS条約に基づく方法で計測された数値である
						</TD>
					</TR>
					<TR>
						<TD nowrap><DIV class=bgb>届出番号　登録番号</DIV></TD>
						<!--<TD><INPUT type=text name="ReportNo" value="<%=gfTrim(ObjRS("ReportNo"))%>" size=35 >　<span style="color:red">※ハイフンなしで入力してください</span></TD></TR>-->
						<TD><INPUT type=text name="ReportNo" value="<%=gfTrim(ObjRS("ReportNo"))%>" size=25 maxlength=18 >　<span style="color:red">※ハイフン付きで入力してください</span></TD></TR>	<!-- 2017/05/07 H.Yoshikawa Upd（size:17⇒25、maxlength:12⇒18、ハイフンなし⇒ハイフン付き） -->
					<!-- 2016/08/08 H.Yoshikawa Add End -->
					<!-- 2017/04/04 H.Yoshikawa Add Start -->
					<TR>
						<TD nowrap><DIV class=bgb>重量　確定事業者</DIV></TD>
						<TD><INPUT type=text name="DefName" value="<%=gfTrim(ObjRS("DefName"))%>" size=80></TD></TR>		<!-- 2017/05/10 H.Yoshikawa Upd(size:40⇒80) -->
	  ...
	  <TABLE>
		  <TR>
		    <TD nowrap><DIV STYLE="FONT-WEIGHT:BOLD;">危険品貨物情報</DIV></TD>
		    <TD></TD></TR>
		  <TR>
		    <!-- 2017/04/04 H.Yoshikawa Del <TD><DIV class=bgb>ＲＨＯ</DIV></TD> -->
		    <TD><INPUT type=hidden name="RHO" value="<%=Trim(ObjRS("RHO"))%>" size=5></TD></TR> <!-- 2017/04/04 H.Yoshikawa Upd text→hidden -->
		   <TR>
		    <TD></TD>
		    <TD>
  ...
  <TABLE border=0 cellPadding=2 cellSpacing=0 width="100%">
	  <TR>
	    <TD colspan=3 align=left valign=bottom>
	    	<BR><INPUT type=checkbox <% if gfTrim(ObjRS("Consent")) = "1" then %>checked<% end if %> disabled><!-- 2017/04/04 H.Yohsikawa Upd 文字色：赤 --><span style="color:red">本画面の入力内容をゲートでの搬入票の代わりとして使用することに同意します。</span>
	    	<BR>　<!-- 2017/04/04 H.Yohsikawa Upd 文字色：赤 --><span style="color:red">※チェックがない場合は仮登録状態であり、予約受付は完了していません。</span>
...
<%End IF%>
       ...
       <!-- </TD><TD> 2017/04/04 H.Yoshikawa Del --> 
       <TABLE border=1 cellPadding=3 cellSpacing=0 align="left" <% '2017/04/04 H.Yoshikawa Upd style="position:relative;left:26px;" %> style="float:right;">
