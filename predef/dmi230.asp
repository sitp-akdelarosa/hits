...
'ブックの搬出完了チェック
  ...
  If ret Then
	'2017/08/25 H.Yoshikawa Upd End
 	'dim OutNum, OdrNum			'2016/10/26 H.Yoshikawa Add
 	dim OutNum(), OdrNum		'2016/10/26 H.Yoshikawa Add
 	Redim OutNum(RowNum-1)
	'2017/08/25 H.Yoshikawa Upd Start
 	...
 	''本数チェック
  	For i=0 To RowNum-1			'2017/05/09 H.Yoshikawa Upd(4⇒RowNum-1)
  		...
  		OutNum(i) = gfTrim(Request("OutNum" & i))
		if gfTrim(Request("UpdFlag" & i)) = "1" then
			...
			'同一属性の搬出済み本数を取得
			'2017/08/25 H.Yoshikawa Add Start (属性が前画面の入力で変更されている可能性があるため、再度取得)
			OutNum(i) = 0
			'同一属性の搬出済み本数を取得
...
function setParam(target){
  ...
  bgset(target);
  window.resizeTo(1366,768); // edited by AK.DELAROSA 2021-01-14
}
      ...
      <!-- 2016/08/23 H.Yoshikawa Upd Start
          ...
          <TD>
          	...
          	%>
				...
				<option value="2" <% if Pcool = "2" then %>selected<% end if %> >無</option>	<!-- 2017/08/25 H.Yoshikawa Add -->
			  </select>
...
<% IF ret Then %>
	...
	<% end if %>
<!-- 2019/01/29 H.Yoshikawa Add-S -->
    <% if Mord=0 then %>
       <P><div class=observ style="text-align:center;">
       		（誓約書）<BR>
	...
	<% end if %>
<!-- 2019/01/29 H.Yoshikawa Add-E -->
       <INPUT type=button value="確定" onClick="GoEntry('No')">
<% Else %>
       <P><DIV class=alert><%=ErrerM%></DIV></P>
<% End If %>
       <INPUT type=button value="戻る" onClick="GoBackT()">
       <INPUT type=hidden name="BackFlag" value="1" >			<!-- 2017/08/25 H.Yoshikawa Add -->
<% IF Mord=0  AND ret Then %>
<!-- 2018/10/16 H.Yoshikawa Add-S -->
<!-- 2019/01/29 H.Yoshikawa Del-S -->
<!--   <span class=observ><INPUT type=checkbox name="ChkRule" value="1" ><a href="../download/download.asp?guide=ヤード内注意事項.pdf">ヤード内注意事項</a>を理解し安全作業を遵守します。</span> -->
<!-- 2019/01/29 H.Yoshikawa Del-E -->
<!-- 2018/10/16 H.Yoshikawa Add-E -->
