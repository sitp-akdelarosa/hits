...
'2009/02/25 Add-S G.Ariola  
	'FieldName=Array("ISNULL(ITC.WorkDate,DATEADD(Year,100,getdate()))","Code1","Name1","ITC.WkNo","CYV.BookNo","ITC.ContNo","CYV.ShipLine","CYV.VslName","CYV.ContSize","CYV.ContHeight","CYV.TareWeight","CYV.ReceiveFrom","BOK.RecTerminal","VSLS.CYCut","ISNULL(ITC.WorkCompleteDate,DATEADD(Year,100,getdate()))","Code2","Name2","Flag1","ITC.Comment1", "ITC.Comment2","ITC.Comment3")
...
'2009/02/25 Upd-S G.Ariola
'  DtTbl(0)=Array("入力日","搬入<BR>予定日","指示元","作業番号","ブッキング番号","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元へ回答","船社","船名","SZ","H","搬入元","CY","CYカット日","備考１","備考２","備考３","備考４","TW")
...
'ADD 20050303 4th "CYV.TareWeight"
	ObjRS.PageSize = 200
	ObjRS.CacheSize = 200
...
'ADD 20050303 START fro Fourth Recon By SEIKO N.Oosige
...
'2010/04/27 M.Marquez Upd-S        
	...
	'2016/10/11 H.Yoshikawa Add End
	'2016/10/12 H.Yoshikawa Add Start
		Kari(i) = gfTrim(ObjRS("kariflag"))
			End If
	'2016/10/12 H.Yoshikawa Add End
		
'        If DtTbl(i)(22)=0 Or isNull(DtTbl(i)(22)) Then
'          DtTbl(i)(22)="-"
'        End If
''ADD 20050303 End
'      '指示先照会済みフラグ
...
'          DtTbl(i)(23) = Trim(ObjRS("TruckerSubCode3")) '2010/04/06 Add M.Marquez
'          DtTbl(i)(24) = Trim(ObjRS("TruckerSubName3")) '2010/04/06 Add M.Marquez
...
'          DtTbl(i)(23) = Trim(ObjRS("TruckerSubCode2")) '2010/04/06 Add M.Marquez
'          DtTbl(i)(24) = Trim(ObjRS("TruckerSubName2")) '2010/04/06 Add M.Marquez
...
'          DtTbl(i)(23) = Trim(ObjRS("TruckerSubCode1")) '2010/04/06 Add M.Marquez
'          DtTbl(i)(24) = Trim(ObjRS("TruckerSubName1")) '2010/04/06 Add M.Marquez
...
<%If Num2<>0 Then%>
	...
	if(IEVersion < 10)
	{
		if((document.body.offsetWidth-10)  < 1118){
			...
			obj3.style.width=document.body.offsetWidth-(document.body.offsetWidth-1215);// Edited by AK.DELAROSA 2021/01/14
			obj3.style.overflowX="auto";
		}
		// Edited by AK.DELAROSA 2021/01/14 START
		// obj3.style.height=document.body.offsetHeight-70;
		// obj3.style.overflowY="scroll";
		var resoHeight = getResolutionHeight();
		var height = document.body.offsetHeight;
		var clientHeight = document.documentElement.clientHeight;

		// if (document.body.offsetHeight > 700) {
		// 	height = document.body.offsetHeight - 300;//3870;
		// }

		if (height < 365) {
			obj3.style.height=365;
			obj3.style.overflowY="scroll";
		} else {
			...
			if (height == 0) {
				...
				height = height - (clientHeight - 500);
				...
				obj3.style.height=document.body.offsetHeight - 100; 
				obj3.style.overflowY="scroll";
			}
			
		}
		// Edited by AK.DELAROSA 2021/01/14 END
	}
	else
	{
	...
	if(IEVersion < 10)
	{
		...
		obj3header.style.height = 51; //35;  // Edited by AK.DELAROSA 2021/01/14
	}
	else
	{
		if((document.body.offsetWidth) >= 1250){
			obj3header.style.width=obj3.clientWidth;//document.body.offsetWidth;
			obj3header.style.height = 35; //35;  // Edited by AK.DELAROSA 2021/01/14
		}
		else
		{
...
// Added by AK.DELAROSA 2021/01/14
...
function GoRenew(sakuNo,bookNo,conNo){
	...
	newWin = window.open("", "ReEntry", "status=yes,width=1350,height=950,left=10,top=10,resizable=yes,scrollbars=yes");
...
function cloneTable(tblSource, tblDestination)
{
	...
	var IEVersion = getInternetExplorerVersion();
	if (IEVersion < 10) {
		source.style.marginTop = "-51px"; //-35px
	} else {
