...
'_/	Modify		:h.matsuda 2020/12/01	コンテナ重複登録を回避
'_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
%>
<% Option Explicit %>
<%
...
'データ取得
  ...
  dim strNum, UpFlag,ret1,ret2,ret3,NG_No_BLCN
...
'hrm add 2020/12/01-s
    If ret2 Then
    'checkComInfoで重複チェックにパスした場合の追加チェック
	  checkImportContByBL_ofContNo ObjConn, ObjRS,strNum, Flag, ret2,NG_No_BLCN
    End If
'hrm add 2020/12/01-e
...
<DIV class=alert>
  ...
  <%ElseIf ret2=false Then%>
    <P>指定されたコンテナ又はBL番号「<%=strNum%>」は<BR>
       既に登録されています。</P>
	  <%If NG_No_BLCN<>"" Then'add 2020/12/01 h.matsuda sta%>
	  	<P>
		  <%If Flag<>4 Then%>
		    BL
	      <%Else%>
		  ...
		  <%End If%>
		番号「<%=NG_No_BLCN%>」</P>
	  <%End If'add 2020/12/01 h.matsuda end%>
  <%Else%>
    <P>指定されたコンテナ又はBL番号「<%=strNum%>」は<BR>
       既に搬出作業を終了しています。</P>
  <%End If%>
