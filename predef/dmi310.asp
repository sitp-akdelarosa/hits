...
'セッションの有効性をチェック
  ...
  '2017/06/16 H.Yoshikawa Add Start (ブッキングとの紐付けチェック追加)
  if ActionType = "B" then
	'DB接続
	dim ObjConn, ObjRS, StrSQL, ErrerFlg, CONnum, BookNo 
	ConnDBH ObjConn, ObjRS

	ErrerFlg = 0
	CONnum = Trim(Request.QueryString("cn"))
	BookNo = Trim(Request.QueryString("bn"))
...
<!-- 2017/06/16 H.Yoshikawa Upd Start
<TITLE><%If ActionType <> "M" Then %>事前登録・搬入票作成<%End If%></TITLE> -->
<TITLE><%If ActionType <> "M" AND ActionType <> "B" Then %>事前登録・搬入票作成<%End If%></TITLE>
<!-- 2017/06/16 H.Yoshikawa Upd End -->
...
<!--
<% If ActionType <> "M" AND ActionType <> "B" Then %>					//2017/06/16 H.Yoshikawa Upd (ActionType <> "B" を追加)
...
// 2016/10/18 H.Yoshikawa Add End
  ...
  if(chkDigit == 0) 
  {
    ...
    //retValue = showModalDialog ("dmi310.asp?CheckDigit=" + chkDigit + "&ActionType=M", window, "dialogWidth:370px; dialogHeight:80px; center:1; scroll: no; dialogTop:300px; ");
     //2017/06/16 H.Yoshikawa Upd Start（ブッキングとの紐付けチェック追加））
     //chengeUpper(target);/
     //target.submit();/
     retValue = showModalDialog ("dmi310.asp?CheckDigit=" + chkDigit + "&ActionType=B&bn=" + targetA[0].value + "&cn=" + targetA[1].value, window, "dialogWidth:450px; dialogHeight:100px; center:1; scroll: no; dialogTop:300px; ");
     if(retValue){
       chengeUpper(target);
       target.submit();
     }
...
function finit(){  
    <% If ActionType <> "M" and ActionType <> "B" Then %>
...
<% elseIf ActionType = "B" Then %>
	<TABLE border=0 cellPadding=3 cellSpacing=7 width="100%">
	<TR>
	<TD colspan=5 align=left>
	<% If ErrerFlg = 0 Then %>
		<div style="color:red">入力されたコンテナ番号、ブッキング番号は間違いありませんか？</div>
	<% Else %>
		<div style="color:red">入力されたコンテナ番号は他のブッキングに紐づいています。ご確認お願いします。</div>
	<% End If %>
	</TD>
	</TR>
	<TR>
	  ...
	  <% End If  %>
	  <TD align=center>
	    <input type="button" name="Stop" value="  戻る  " Onclick="fStop();" onkeypress="return true">
	  </TD>
	</TR>
	</TABLE>
