...
'	end if	
	...
	if Key <> "" then 
		if FieldName(CInt(Key)) = "WorkDate" AND SortKey = "ASC" then 
...
'20030911  DtTbl(0)=Array("入力日","搬入予定日","指示元","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元への回答","船社","船名","サイズ","返却先","ディテンション<BR>フリータイム","備考")
	DtTbl(0)=Array("入力日","搬入<BR>予定日","指示元","コンテナ番号","完了日時","指示先","指示先<BR>回答","照会先Frag","指示元への回答","船社","船名","SZ","返却先","ディテンション<BR>フリータイム","備考","作業管理番号","コード","指示元<BR>担当")
	dim ctr
	for ctr = 1 to 3
...
'3th Add End
	...
	'データ取得
	StrSQL = "SELECT T.* FROM (SELECT ITC.InputDate, "&_
			...
			"ITC.RegisterCode, ITC.TruckerSubCode1, ITC.TruckerSubCode2, "&_			
			"ITC.TruckerSubCode3, ITC.TruckerSubCode4,ITC.ContNo,"&_			
