...
'データをDBより検索
  ...
  dim DefName, EntryName									'2017/04/04 H.Yoshikawa Add
...
if WkNo <> "" then
	...
	If ret Then
		...
		strSQL=" select top 1"
		'2017/07/31 H.Yoshikawa Add Start サイズ、タイプ、ハイトは自身のコンテナ情報から取得
		...
		EntryName  = gfTrim(ObjRS_CYV("EntryName"))			'指示元担当者	2017/04/04 H.Yoshikawa Add
		TruckerFlag=0
		...
		if ReportNo <> "" then
			StrSQL = "SELECT DefName From mDefTrade "&_
			       "WHERE DefCode = '"  &gfSQLEncode(ReportNo) & "'"
			ObjRS.Open StrSQL, ObjConn
			if err <> 0 then
				DisConnDBH ObjConn, ObjRS	'DB切断
				jampErrerP "1","b401","01","実搬入：確定事業者データ取得","102","SQL:<BR>"&StrSQL
			end if
			if not ObjRS.EOF then
