...
<%
	...
	Select Case  str_show_column
		Case "1"
			...
			'2010/05/05 Upd-S C.Pestano
			ReDim FieldKey(13)			
			FieldKey=Array("入力日","指示元","指示元へ回答","回答","ブッキング番号","ピック済","SZ","タイプ","H","材質","船社","船名","指示先","指示先回答")	
		Case "2"
		    'Y.TAKAKUWA 2013-02-20 Upd-S
			ReDim FieldKey(19)
			FieldKey=Array("搬入票出力","ロック指示先", "ロック中ヘッドID", "搬出入予定日","指示元","指示元へ回答","回答","作業番号","コンテナ番号/BL番号","船社","船名","SZ","搬入元","CY","搬出許可日","フリータイム","CYカット日","指示先","指示先回答")		
			'Y.TAKAKUWA 2013-02-20 Upd-E
			'2010/05/05 Upd-E C.Pestano
	end select
...
function fInit(){
	...
	}else if("<%=str_show_column%>" == "2"){ 
		for(i=0;i<19;i++){                            //Y.TAKAKUW Upd from 17
			fCheck(str,i,"Chk_Sel"+i,"Chk_SelDis"+i);
		}
	}
}
...
function fFormatPage(){
	...
	if("<%=str_show_column%>" == "1"){  
		createCookie('HitsTbl1', str, 14)
	}else{
		createCookie('HitsTbl2', str, 19) //Y.TAKAKUWA Upd from 17
	}
...
function fClear(){
	...
	if("<%=str_show_column%>" == "1"){  
		createCookie('HitsTbl1', "", 14)
	}else{
		createCookie('HitsTbl2', "", 19) //Y.TAKAKUW Upd from 17
	}
	opener.document.frm.submit();
	window.close();
}
